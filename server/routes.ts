...
const logActivity = async (userId: number, action: string, details: any = {}, req?: any) => {
  ...
  app.get("/api/auth/me", requireAuth, async (req: any, res) => {
    try {
      ...
      res.json({ user: { id: user.id, email: user.email, username: user.username, level: user.level, xp: user.xp, role: user.role, interfaceMode: user.interfaceMode } });
    } catch (error: any) {
      res.status(500).json({ message: error.message });
    }
  });
  ...
  app.patch("/api/user/interface-mode", requireAuth, async (req: any, res) => {
    try {
      const { interfaceMode } = req.body;
      const validModes = ['wellness', 'productivity', 'trader', 'team', 'therapy'];
      if (!validModes.includes(interfaceMode)) {
        return res.status(400).json({ message: "Invalid interface mode" });
      }
      await db.update(users).set({ interfaceMode }).where(eq(users.id, req.session.userId));
      res.json({ success: true, interfaceMode });
    } catch (error: any) {
      res.status(500).json({ message: error.message });
    }
  });
